...
fun validateAndNormalizeBigDecimal(value: BigDecimal?, scale: Int, minimum: BigDecimal, maximum: BigDecimal, propertyName: String): BigDecimal {
    if (value == null)
        throw IllegalArgumentException("null $propertyName")

    val normalized = try {
        value.setScale(scale, RoundingMode.UNNECESSARY)
    } catch(e: ArithmeticException) {
        throw IllegalArgumentException("$propertyName ($value) has too many decimals")
    }

    if (normalized < minimum || normalized > maximum)
        throw IllegalArgumentException("$propertyName ($value) is out of range $minimum .. $maximum")

    return normalized
}

fun validateAndConvertToBigDecimal(value: Double, scale: Int, scaleMultiplier: Long, minUnscaled: Long, maxUnscaled: Long, propertyName: String): BigDecimal {
    if (value.isNaN())
        throw IllegalArgumentException("NaN $propertyName")

    val unscaled = Math.round(value * scaleMultiplier)

    if (unscaled < minUnscaled || unscaled > maxUnscaled) {
        val minimum = minUnscaled / scaleMultiplier.toDouble()
        val maximum = maxUnscaled / scaleMultiplier.toDouble()
...
class DecimalValidator(private val propertyName: String, totalDigits: Int, decimalDigits: Int) {
    private val scale: Int
    private val minimum: BigDecimal
    private val maximum: BigDecimal
    private val scaleMultiplier: Long
    private val minUnscaled: Long
    private val maxUnscaled: Long
    val normalizedZero: BigDecimal

    init {
        if (decimalDigits >= totalDigits)
            throw IllegalArgumentException("$decimalDigits >= $totalDigits")

        if (totalDigits !in 1..18)
            throw IllegalArgumentException("$totalDigits !in 1..18")

        if (decimalDigits < 0)
            throw IllegalArgumentException("$decimalDigits < 0")


        scale = decimalDigits
        minimum = BigDecimal.ZERO
        maximum = BigDecimal.valueOf("9".repeat(totalDigits).toLong(), scale)
        scaleMultiplier = (("1" + "0".repeat(decimalDigits)).toLong())
        minUnscaled = 0
        maxUnscaled = maximum.unscaledValue().longValueExact()

        normalizedZero = validateAndNormalize(BigDecimal.ZERO)
    }

    fun validateAndNormalize(value: BigDecimal?): BigDecimal {
        return validateAndNormalizeBigDecimal(value, scale, minimum, maximum, propertyName)
    }

    fun validateAndConvert(value: Double): BigDecimal {
        return validateAndConvertToBigDecimal(value, scale, scaleMultiplier, minUnscaled, maxUnscaled, propertyName)
    }
}

fun validate8DigitTaxNumber(taxNumber: String, propertyName: String): Int? {
    if (taxNumber.length != 8)
        throw IllegalArgumentException("$propertyName must be 8 digits")

    for (i in 0..7)
        if (taxNumber[i] < '0' || taxNumber[i] > '9')
            throw IllegalArgumentException("$propertyName must be 8 digits")

    return Integer.valueOf(taxNumber)
}

fun validate8DigitTaxNumber(taxNumber: Int, propertyName: String): Int {
    if (taxNumber in 10000000..99999999)
        return taxNumber

    throw IllegalArgumentException("$propertyName must be exactly 8 digits")
}
